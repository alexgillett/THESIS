...
#####################################################################
# 4. EXACTrisk_working_functionA: internal function for risk estimation
# for method A, when there is only 1 polygenic risk score value under
# consideration.
# Using method A, this function estimates the risk of disease for an
# individual, {I}, given a polygenic risk score for {I} (mI), genotypes 
...
### tau = vector. Length = XXX. Contains the effect sizes on the liability
...
### r = numeric. Coefficient of relatedness between {I} and {R}
### theta = numeric. Coefficient of co-ancestry between {I} and {R} XXX
...
#####################################################################
# OUTPUT:
### Function outputs a vector of length 2. 
### output[1] = the joint probability 
# of Y_{I} = 1 and Y_{R} = y_{R} given the observed environmental and genetic
# variables for {I} and {R}, and,
### output[2] = the probability that Y_{R} = y_{R} given the observed
# environmental and genetic variables for {I} and {R}
# y_{R} = 0 or 1
EXACTrisk_working_functionA <- function(mI, g_Ivec, e_Ivec, g_Rvec, e_Rvec, maf.vec, levelsE, pEmat, D_threshold, tau, VM, VhG, VhE, H2, h2=H2, VAhG=VhG, VDhG=0, r=0.5, theta=0.25, YR=1){	
	...
	mu <- mean_mI_f(tau=tau, x_I=x_Iv, x_R=x_Rv, r=r, mI=mI)
...
#####################################################################
# 5. EXACTrisk_working_function2A: internal function for risk estimation
# for method A, when there is are multiple polygenic risk score value 
...
#####################################################################
# INPUTS:
### Inputs are as described above for function: EXACTrisk_working_functionA
# except:
# x = vector. A vector containing polygenic risk score values. 
### Again, some inputs require calculation. Please see Chapter 4 examples
# for example code to calculate inputs such as tau and D_threshold, which
# may not be obvious without having read the thesis.
#####################################################################
# OUTPUT:
...
# y_{R} = 0 or 1
EXACTrisk_working_function2A <- function(x, g_Ivec, e_Ivec, g_Rvec, e_Rvec, maf.vec, levelsE, pEmat, D_threshold, tau, VM, VhG, VhE, H2, h2=H2, VAhG=VhG, VDhG=0, r=0.5, theta=0.25, YR=1){	
	x_Iv <- rep(0, length(tau))
	x_Rv <- x_Iv
	n_E <- length(levelsE)
	n_G <- length(maf.vec)
...
riskLTMM_methodA <- function(mIvec, g_Ivec, e_Ivec, maf.vec, levelsE, pEmat, D_threshold, tau, VM, VhG, VhE, H2, h2=H2, VAhG=VhG, VDhG=0, r=0.5, theta=0.25, combosR_E, R="P"){
	n_E <- length(levelsE)
	n_G <- length(maf.vec)
	df_G <- NULL
	for(i in 1:n_G){
	...
	for(i in 1:dim(combosR_G)[1]){
		g_Rveci <- unlist(combosR_G[i,])
		names(g_Rveci) <- NULL
		pGRi <- rep(0, n_G)
		for(k in 1:n_G){
			GRk <- g_Rveci[k] 
			temp.pGk <- jointG_I_R_function(G_I=g_Ivec[k], R="P", maf=maf.vec[k])
		...
		for(j in 1:dim(combosR_E)[1]){
			...
			pERj <- prod(pERj)			
			probs1ij <- t(apply(mImat, 1, EXACTrisk_working_function2A, g_Ivec= g_Ivec, e_Ivec= e_Ivec, g_Rvec= g_Rveci, e_Rvec= e_Rvecj, maf.vec= maf.vec, levelsE= levelsE, pEmat= pEmat, D_threshold = D_threshold, tau= tau, VM=VM, VhG= VhG, VhE= VhE, H2= H2, h2=h2, VAhG= VAhG, VDhG= VDhG, r=r, theta=theta, YR=1))
			probs0ij <- t(apply(mImat, 1, EXACTrisk_working_function2A, g_Ivec= g_Ivec, e_Ivec= e_Ivec, g_Rvec= g_Rveci, e_Rvec= e_Rvecj, maf.vec= maf.vec, levelsE= levelsE, pEmat= pEmat, D_threshold = D_threshold, tau= tau, VM=VM, VhG= VhG, VhE= VhE, H2= H2, h2=h2, VAhG= VAhG, VDhG= VDhG, r=r, theta=theta, YR=0))
	...
	output <- cbind(affected.numer/affected.denom, unaffected.numer/unaffected.denom)
	output
}
#####################################################################
### EXAMPLE OF USAGE ################################################
