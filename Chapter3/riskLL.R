...
# we wish to estimate risk (all denoted by {I}). This PRS has been
# standardised such that the mean = 0 and variance = 1
...
### VM = the variability of the (unstandardised) polygenic risk score, 
# calculated using the estimated relative risk of disease for each included
# SNP  
### R = character. The relative type of individual {R}. 
# Can equal: "S" = sibling, "P" = parent, "O" = offspring, 
# "GP" = grandparent, "Av" = avuncular (aunt/ uncle) (by blood),
...
# mIvec <- round(quantile(rnorm(100000, mean=0, sd=1), probs = c(0.25, 0.5, 0.75)), digits=2)
